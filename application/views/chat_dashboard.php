...
<head>
    <meta charset="utf-8">
    <title>Secure Chat | Dashboard</title>
...
<div id="container">
    <h1>
        Welcome, <?php echo $this->user->name; ?>!
        <span style="float: right">Browser: <?php echo $this->agent->browser().' '.$this->agent->version(); ?></span>
    </h1>
    ...
    <div id="body">
        ...
        <table style="width: 100%" id="table-friend">
            ...
            <tr>
                <td><a href="javascript:;" data-friend="<?php echo $item->id; ?>"><?php echo $item->name; ?></a></td>
            </tr>
            <?php } ?>
        </table>
		<?php } else { ?>
		<p>It's lonely here... <br/> Why not to try invite more <a href="<?php echo base_url(); ?>">here</a>?</p>
		<?php } ?>
        <br />
        <br />
...
<script type="text/x-template" id="msg-template" style="display: none">
    <tbody>
        <tr class="msg-wgt-message-list-header">
            <td rowspan="2"><img src="<?php echo base_url('assets/avatar.png'); ?>"></td>
            <td class="name"></td>
            <td class="time"></td>
        </tr>
                ...
                $.post('<?php echo site_url('chat/getChats'); ?>', {chatWith: $data.chatWith}, function(data, textStatus, xhr) {
                    ...
                    $.each(data.chats, function(index, el) {
                        ...
                        if ($that.find('#'+ id).length == 0) {
                            ...
                            tplBody.find('.msg-wgt-message-list-body > td').html(nl2br(val.messages)); // set message
